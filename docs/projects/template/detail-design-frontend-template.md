...
|-----------|------------|-----------|------|
| close | なし | モーダルを閉じる時 | 親コンポーネントにモーダルを閉じることを通知 |
| todoUpdated | Todo | チケット更新後 | 更新されたチケットデータを親に伝える |
-->
...
|------|-----|---------|-----------|-----------|
| チケット詳細取得 | /api/todos/{id} | GET | loadTodoDetail() | モーダル表示時 |
| チケット完了切替 | /api/todos/{id}/toggle | PATCH | handleToggleComplete() | 完了ボタンクリック時 |
-->
...
<!--
フォーム入力のバリデーションルールを記載
例:

| 項目 | ルール | エラーメッセージ | 検証タイミング |
|------|--------|----------------|--------------|
| コメント内容 | 必須、1〜2000文字 | "コメントを入力してください（最大2000文字）" | 投稿ボタンクリック時 |
| 投稿者 | 必須 | "投稿者を選択してください" | 投稿ボタンクリック時 |
-->

### 6.2 バリデーション実装

<!--
バリデーションの実装方法を簡潔に記載（オプション）
例:
```typescript
...
async function handleSubmit() {
  formState.errors = validateCreateCommentRequest({
