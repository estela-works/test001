...
| 画面名 | ユーザー管理画面 |
| パス | `/users` |
| Vueコンポーネント | `src/frontend/src/views/UserView.vue` |
| 子コンポーネント | UserForm, UserCard |
| 状態管理 | Pinia (`useUserStore`) |
...
| 遷移元 | トリガー | 遷移先 |
|--------|---------|--------|
| SC-003: プロジェクト画面 | router-link | 本画面 |
...
| 日付 | 変更内容 | 関連案件 |
|------|----------|----------|
| 2025-12-24 | Vue.js 3 移行（UserView.vue + 子コンポーネント） | 202512_Vue.js移行 |
